...
from .MobileNetV2_ops import basic_blocks, super_blocks, PlainNet as _PlainNet
...
class PlainNet(_PlainNet):
    def __init__(self, argv=None, opt=None, num_classes=None, plainnet_struct=None, no_create=False,
                 no_reslink=None, no_BN=None, use_se=None, dropout=None, input_resolution=None,
                 **kwargs):
        ...
        if input_resolution is None:
            if module_opt is not None:
                self.input_resolution = module_opt.input_resolution
            else:
    ...
    def extract_stage_features_and_logit(self, x, target_downsample_ratio=None):
        x = F.interpolate(x, self.input_resolution, mode='bilinear')
        stage_features_list = []
    ...
    def forward(self, x, pre_GAP=False):
        x = F.interpolate(x, self.input_resolution, mode='bilinear')
        output = x
        for block_id, the_block in enumerate(self.module_list):
            output = the_block(output)
            if self.dropout is not None:
                dropout_p = float(block_id) / len(self.module_list) * self.dropout
                output = F.dropout(output, dropout_p, training=self.training, inplace=True)

        if pre_GAP: return output
    ...
    def get_FLOPs(self, input_resolution=None):
        if input_resolution is None: input_resolution = self.input_resolution
