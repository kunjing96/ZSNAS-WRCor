...
from . import _get_right_parentheses_index_, _create_netblock_list_from_str_
...
class Sequential(PlainNetBasicBlockClass):
    ...
    def forward(self, x):
        output = x
        for inner_block in self.module_list:
...
class MultiSumBlock(PlainNetBasicBlockClass):
    ...
    def forward(self, x):
        output = self.module_list[0](x)
        for inner_block in self.module_list[1:]:
